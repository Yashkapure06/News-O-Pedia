<template>
    <div>
        ...
        <section class="right-side-bar">
            ...
            <ul>
                <li class="timebox-side" v-for="(techArticle, index) in techArticles" :key="index">
                    <router-link class="links" to="/science"> <h5>{{ techArticle.title }}</h5></router-link>
                    <img :src="techArticle.urlToImage" alt="Article image">
                </li>
            </ul>
        </section>
...
export default {
    ...
    mounted() {
        ...
        .then(response => {
            this.techArticles = response.data.articles;
        })
        .catch(error => {
            console.log(error);
        });
